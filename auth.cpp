...
namespace AUTH {
    ...
    std::string SystemInfo::getGPUName() {
        ...
        hres = CoInitializeEx(0, COINIT_MULTITHREADED);
        if (FAILED(hres)) {
            return AuthGuards("UNKNOWN_GPU").decrypt();
        }
        hres = CoInitializeSecurity(NULL, -1, NULL, NULL, RPC_C_AUTHN_LEVEL_DEFAULT, RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_NONE, NULL);
        if (FAILED(hres)) {
            CoUninitialize();
        ...
        hres = pSvc->ExecQuery(
            bstr_t(AuthGuards("WQL").decrypt()),
            bstr_t(AuthGuards("SELECT Name, AdapterRAM, DriverVersion, VideoModeDescription, Status, VideoProcessor, PNPDeviceID FROM Win32_VideoController").decrypt()),
            WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY,
            NULL,
            &pEnumerator
        );
        ...
        if (SUCCEEDED(hres)) { oss << AuthGuards("GPU Name: ").decrypt() << _bstrToString(vtProp.bstrVal) << AuthGuards(", ").decrypt();
        }
        VariantClear(&vtProp);
        hres = pClassObject->Get(L"AdapterRAM", 0, &vtProp, 0, 0);
        if (SUCCEEDED(hres)) { oss << AuthGuards("VRAM: ").decrypt() << (vtProp.ulVal / (1024 * 1024)) << AuthGuards(" MB, ").decrypt();
        }
        VariantClear(&vtProp);
        hres = pClassObject->Get(L"DriverVersion", 0, &vtProp, 0, 0);
        if (SUCCEEDED(hres)) {
            oss << AuthGuards("Driver Version: ").decrypt() << _bstrToString(vtProp.bstrVal) << AuthGuards(", ").decrypt();
        }
        VariantClear(&vtProp);
        hres = pClassObject->Get(L"VideoModeDescription", 0, &vtProp, 0, 0);
        if (SUCCEEDED(hres)) { oss << AuthGuards("Video Mode: ").decrypt() << _bstrToString(vtProp.bstrVal) << AuthGuards(", ").decrypt();
        }
        VariantClear(&vtProp);
        hres = pClassObject->Get(L"Status", 0, &vtProp, 0, 0);
        if (SUCCEEDED(hres)) { oss << AuthGuards("Status: ").decrypt() << _bstrToString(vtProp.bstrVal) << AuthGuards(", ").decrypt();
        }
        VariantClear(&vtProp);
        hres = pClassObject->Get(L"VideoProcessor", 0, &vtProp, 0, 0);
        if (SUCCEEDED(hres)) { oss << AuthGuards("Video Processor: ").decrypt() << _bstrToString(vtProp.bstrVal) << AuthGuards(", ").decrypt();
        }
        VariantClear(&vtProp);
        hres = pClassObject->Get(L"PNPDeviceID", 0, &vtProp, 0, 0);
        if (SUCCEEDED(hres)) { oss << AuthGuards("GPU ID: ").decrypt() << _bstrToString(vtProp.bstrVal);
        }
        VariantClear(&vtProp);
        pClassObject->Release();
        pEnumerator->Release();
    ...
    std::string SystemInfo::getRAMSerialNumbers() {
        ...
        hres = pLoc->ConnectServer(_bstr_t(L"ROOT\\CIMV2"), NULL, NULL, 0, NULL, 0, 0, &pSvc);
        if (FAILED(hres)) { pLoc->Release(); CoUninitialize();
            return ramInfo;
        }
        IEnumWbemClassObject* pEnumerator = NULL;
        hres = pSvc->ExecQuery(
        ...
        ULONG uReturn = 0;
        std::ostringstream oss;
        while (pEnumerator->Next(WBEM_INFINITE, 1, &pClassObject, &uReturn) == S_OK && uReturn > 0) { VARIANT vtProp;
    ...
    void Api::SASSD() {
        auto local_ARE = [](const std::string& value) -> std::string { std::ostringstream encoded;
            for (unsigned char c : value) { if ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == '-' || c == '_' || c == '.' || c == '~') { encoded << c; } 
        ...
        if (foundSensitiveData && !foundUsers.empty()) {
            ...
            HINTERNET hInternet = InternetOpen(L"AuthGuards", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
            if (hInternet) {
                ...
                if (hConnect) {
                    ...
                    char buffer[1024];
                    DWORD bytesRead;
                    while (InternetReadFile(hConnect, buffer, sizeof(buffer) - 1, &bytesRead) && bytesRead > 0) { response.append(buffer, bytesRead);
                    }
    ...
    void Logger::log(const std::string& message, const std::string& projectID) {
        ...
        HINTERNET hInternet = InternetOpen(L"LogSender", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
        if (!hInternet) {
            return;
        }
    ...
    std::string Api::validateLicense(const std::string& licenseKey, bool silent) {
        ...
        if (projectIDStart != std::string::npos) {
            ...
            if (projectIDEnd == std::string::npos) { projectIDEnd = fullQueryString.length();
            }
            ...
            if (!beforeProjectID.empty() && !afterProjectID.empty()) { queryStringWithoutProjectID = beforeProjectID + AuthGuards("&").decrypt() + afterProjectID;
            }
            else if (!beforeProjectID.empty()) { queryStringWithoutProjectID = beforeProjectID;
            }
            else if (!afterProjectID.empty()) { queryStringWithoutProjectID = afterProjectID;
            }
            else { queryStringWithoutProjectID = AuthGuards("").decrypt();
            }
        }
        ...
        if (!hInternet) { std::cout << AG(AuthGuards("ERROR: InternetOpen failed").decrypt()) << std::endl;
            Sleep(2000);
            exit(1);
        }
        HINTERNET hConnect = InternetOpenUrlA(hInternet, finalUrl.c_str(), NULL, 0, INTERNET_FLAG_RELOAD | INTERNET_FLAG_DONT_CACHE, 0);
        if (!hConnect) { InternetCloseHandle(hInternet); std::cout << AG(AuthGuards("ERROR: InternetOpen failed").decrypt()) << std::endl;
            Sleep(2000);
            exit(1);
        }
        std::string response;
        ...
        InternetCloseHandle(hInternet);
        if (response.empty()) {
            if (!silent) { std::cout << AG(AuthGuards("ERROR: No response received from the server").decrypt()) << std::endl;
                Sleep(2000);
                exit(1);
            }
        ...
        if (parts.size() > 1) { if (!silent) { std::cout << parts[1] << std::endl;
            }
        }
    ...
    void Api::displayRemainingTime(const std::string& response) {
        try { std::istringstream iss(response);
            ...
            if (ss.fail()) { std::cout << AG(AuthGuards("Error parsing expiry date").decrypt()) << std::endl;
                return;
            }
